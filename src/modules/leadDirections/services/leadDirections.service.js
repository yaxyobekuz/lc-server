...
export const getById = async (id) => {
  const doc = await LeadDirection.findById(id);
...
export const create = async ({ name }) => {
  ...
  const exists = await LeadDirection.findOne({
    name: trimmed,
    isActive: true,
  });
  if (exists) throw new ApiError(409, "Bunday yo'nalish mavjud");
  return LeadDirection.create({ name: trimmed });
};
...
export const update = async (id, body) => {
  ...
  if (body.name !== undefined) {
    ...
    if (trimmed !== doc.name) {
      const conflict = await LeadDirection.findOne({
        _id: { $ne: doc._id },
        name: trimmed,
        isActive: true,
      });
      if (conflict) throw new ApiError(409, "Bunday yo'nalish mavjud");
    }
    doc.name = trimmed;
  }
