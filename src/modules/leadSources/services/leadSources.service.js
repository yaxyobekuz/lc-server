...
export const getById = async (id) => {
  const doc = await LeadSource.findById(id);
...
export const create = async ({ name }) => {
  ...
  const exists = await LeadSource.findOne({ name: trimmed, isActive: true });
  if (exists) throw new ApiError(409, "Bunday lead manba mavjud");
  return LeadSource.create({ name: trimmed });
};
...
export const update = async (id, body) => {
  ...
  if (body.name !== undefined) {
    ...
    if (trimmed !== doc.name) {
      ...
      if (conflict) throw new ApiError(409, "Bunday lead manba mavjud");
    }
    doc.name = trimmed;
  }
